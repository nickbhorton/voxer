...
void add_cube(std::vector<uint32_t>& positions, std::array<float, 3> position, uint8_t cidx)
{
    ...
    for (size_t i = 0; i < vertex_count; i++) {
        positions.push_back(pack_float(positions_gen[position_indexes[i]] + position, cidx));
    }
}
...
int main(int argc, char** argv)
{
    ...
    ShaderProgram basic_s{
        {{"res/shaders/4.vert.glsl", GL_VERTEX_SHADER}, {"res/shaders/1.frag.glsl", GL_FRAGMENT_SHADER}}
    };
        ...
        for (size_t y = 0; y < 4; y++) {
            for (size_t x = 0; x < 4; x++) {
                ...
                for (auto const& p : obj_positions) {
                    ...
                    add_cube(position_v, position, p[3]);
                }

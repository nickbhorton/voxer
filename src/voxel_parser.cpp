...
void read_chunk(std::ifstream& file_stream, Chunk& chunk, const char* header)
{
    char chunk_id[4];
    file_stream.read(chunk_id, 4);
    assert(strncmp(chunk_id, header, 4) == 0);
    file_stream.read(
...
parse_vox_file(std::string const& file_name)
{
    ...
    // MAIN chunk
    Chunk main{};
    read_chunk(file_stream, main, "MAIN");
    assert(main.chunk_content_count == 0);
    // TODO PACK chunk
    // SIZE chunk
    Chunk size{};
    read_chunk(file_stream, size, "SIZE");
    assert(size.chunk_content_count == 12);
    assert(size.child_content_count == 0);
    std::array<int32_t, 3> size_data{};
    for (size_t i = 0; i < 3; i++) {
        file_stream.read(
            reinterpret_cast<char*>(&size_data[i]),
            sizeof(size_data[i])
        );
    }
    ...
    for (int32_t i = 0; i < number_voxels; i++) {
        for (size_t j = 0; j < 4; j++) {
            file_stream.read(
                reinterpret_cast<char*>(&voxels[i][j]),
                sizeof(voxels[i][j])
            );
        }
    ...
    if (file_stream.eof()) {
        ...
        Chunk rgba{};
        read_chunk(file_stream, rgba, "RGBA");
        for (int i = 0; i < 256; i++) {
            for (int j = 0; j < 4; j++) {
                file_stream.read(
